...
int main(int argc, char **argv) {
  ...
  for (int len = 1; len < vertice_number; len++) {
    for (int i = 0; i < vertice_number; i++) {
      if (i + len < vertice_number)
        maxPTable[i][i + len] =
            max_finder(i, len, maxPTable, lineTable, linkPTable);
    }
  }
  ...
  vector<line> lineSet;
  link_tracer(0, vertice_number - 1, maxPTable, lineTable, linkPTable,
              reverseTable, lineSet);
...
int max_finder(int start, int len, int **maxptable, int *line_table,
               int **linkptable) {
  int end = start + len;
  if (maxptable[start][end] != -1)
    return maxptable[start][end];
  int result = 0;
  ...
  if (crosser > start + len || crosser < start) {
    ...
    } else if (result < 1 + maxptable[start][crosser - 1] +
                            maxptable[crosser + 1][start + len - 1]) {
      result = 1 + maxptable[start][crosser - 1] +
               maxptable[crosser + 1][start + len - 1];
      linkptable[start][start + len] = crosser;
    }
...
int link_tracer(int start, int end, int **maxptable, int *line_table,
  ...
  if (s - 1 >= start)
    link_tracer(start, s - 1, maxptable, line_table, linkptable, reversetable,
                line_set);
  if (s + 1 < line_table[s] - 1)
    link_tracer(s + 1, line_table[s] - 1, maxptable, line_table, linkptable,
                reversetable, line_set);
  if (line_table[s] + 1 <= end)
    link_tracer(line_table[s] + 1, end, maxptable, line_table, linkptable,
                reversetable, line_set);
  return 0;
}
