...
vector<int> findPair(vector<pair<int, int>> &data, int n) {
  ...
  call the findChord function to  trace back the chords and return the result as
  a vector*/

  vector<vector<unsigned short>> m(n, vector<unsigned short>(n, 0));
  // m is for storing count of chords
  vector<vector<char>> s(n, vector<char>(n, 1));
  // s is for saving which case, initialize with case1
  ...
  for (size_t d = 1; d < n; ++d) {
    for (size_t i = 0; i < n - d; ++i) {
      ...
      if (k < i || k > j) {
        ...
        // case3 (but what if m[i][j-1] = m[i][k-1]+ 1 + m[k+1][j-1])
  ...
  // find from (0, n - 1) trace back the chord
...
void findChord(vector<pair<int, int>> &data, vector<int> &result,
  ...
  // if case2: k is i, so we must choose k
  if (s[i][j] == 2) {
