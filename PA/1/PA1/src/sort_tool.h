...
private:
  void QuickSortSubVector(vector<int> &, int, int,
                          const int); // quick sort subvector
  int RandomizedPartition(vector<int> &, int,
                          int);           // randomized partition the subvector
  ...
  void MaxHeapify(vector<int> &,
                  int); // make tree with given root be a max-heap
